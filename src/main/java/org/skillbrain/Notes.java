package org.skillbrain;

public class Notes {
    //  puse de mentor in chat

//   mentor SkillBrain19:33
//    driver.executeScript("arguments[0].scrollIntoView(true);", submitButton);
//    Mentor SkillBrain19:44
//   WebElement submitButton = driver.findElement(By.cssSelector(".btn.btn-primary"));
//     driver.executeScript("arguments[0].scrollIntoView(true);", submitButton);
//   Mentor SkillBrain20:04
    //label[@for='tree-node-desktop']//span[@class='rct-checkbox']//*[name()='svg']
//  Mentor SkillBrain20:08
//   driver.findElement(By.xpath("(//li[@id='item-1'])[1]")).click();
//  Thread.sleep(2000);
//   driver.findElement(By.cssSelector(".rct-icon.rct-icon-expand-close")).click();
//  driver.findElement(By.xpath("//label[@for='tree-node-desktop']//span[@class='rct-checkbox']//*[name()='svg']")).click();
//  Thread.sleep(2000);
//  System.out.println(driver.findElement(By.id("result")).getText());
//  (//li[@id='item-2'])[1]
//        xpath
//   (//label[@class='custom-control-label'])[1]
//  System.setProperty("webdriver.chrome.driver" , "drivers/chromedriver");
//          ChromeDriver driver = new ChromeDriver();
//        driver.manage().window().maximize();
//       driver.get("https://demoqa.com/");
//  https://demoqa.com/buttons
    //  button[normalize-space()='Click Me']
              //curs selenium 2
//CCNA   CURS PT DOCUMENTARE
    //Mentor SkillBrain20:12
    //https://mvnrepository.com/artifact/commons-io/commons-io/2.11.0

    //public void takeScreenshot(WebDriver driver) throws IOException {
    //
    //        TakesScreenshot screenshot = (TakesScreenshot) driver;
    //        File source = screenshot.getScreenshotAs(OutputType.FILE);
    //        String destinationFile = System.getProperty("user.dir") +"/Logs/printScreen.png";
    //        FileUtils.copyFile(source, new File(destinationFile));
    //
    //    }

    //public static void takeScreenshot(WebDriver driver) throws IOException {
    //        String destinationFile = null;
    //        TakesScreenshot screenshot = (TakesScreenshot) driver;
    //        File source = screenshot.getScreenshotAs(OutputType.FILE);
    //        destinationFile = System.getProperty("user.dir")
    //                + "/Logs/printScreen-" + LocalDateTime.now() + ".png";
    //        FileUtils.copyFile(source, new File(destinationFile));
    //    }

    //git pull=comanda pt modificate git hub

    //git pull --rebase origin master pt actualizare git

    //
    //   String DATE_FORMATTER= "yyyy-MM-dd-HHmmss";
    //       // System.out.println("Current Date="+today_time);
    //        DateTimeFormatter formatter = DateTimeFormatter.ofPattern(DATE_FORMATTER);
    //        String formatDateTime = LocalDateTime.now().format(formatter);
    //        System.out.println("Formatted Time :" +formatDateTime);
    //FileUtils.copyFile(source, new File(System.getProperty("user.dir")+"\\Logs\\print-"+ formatDateTime + ".png"));
//https://chromedriver.chromium.org/capabilities   pt adaugare extensie la Chrome
//.btn.btn-primary.js-accept.gtm_h76e8zjgoo.btn-block   LINK PRIM TEST
//.gdpr-cookie-banner.js-gdpr-cookie-banner.pad-sep-xs.pad-hrz-none.show    LINK SECOND TEST
//.js-dismiss-login-notice-btn.dismiss-btn.btn.btn-link.pad-sep-none.pad-hrz-none   3 link test pt login emag
////*[contains(text(),'Salut!')]   link salut
//https://www.guru99.com/chrome-options-desiredcapabilities.html       pt configurari la teste
//https://www.browserstack.com/guide/page-object-model-in-selenium   pt browser sele
//private final static String GO_TO_REGISTRATION ="//a[normalize-space()='Go To Registration']";
//private final static String BANNER_IMAGE =".banner-image"; dEMOQUA
//private final static String OPTION_ROMANIA ="option[value='70']"
//String firstPageId = driver.getWindowHandle();
//Set<String> tabIDs = driver.getWindowHandles();
//Iterator<String> iter = tabIDs.iterator();
//Set<String> tabIDs = driver.getWindowHandles();
//            System.out.println(tabIDs.size());
//            Iterator<String> iter = tabIDs.iterator();
//            String parentid = iter.next();
//            String childId = iter.next();
//            driver.switchTo().window(childId);
//NEXT
//
//driver.get("https://www.emag.ro/");
//            EmagHomePage homePage = new EmagHomePage(driver);
//            //homePage.closeOfertaZileiButton.click();
//            homePage.gdprFooter.click();
//            homePage.acceptButton.click();
//            driver.navigate().refresh();
//            WebDriverWait wait = new WebDriverWait(driver, Duration.ofSeconds(5));
//            wait.until(ExpectedConditions.visibilityOf(homePage.gdprFooter)); // 2 s
//            homePage.acceptButton.click();
    //  driver.get("https://www.emag.ro/");
    //  EmagHomePage homePage = new EmagHomePage(driver);
    //homePage.closeOfertaZileiButton.click();
    // homePage.gdprFooter.click();
    //   homePage.acceptButton.click();
    //  driver.navigate().refresh();
    // WebDriverWait wait = new WebDriverWait(driver, Duration.ofSeconds(5));
    //wait.until(ExpectedConditions.visibilityOf(homePage.gdprFooter)); // 2 s
    //  homePage.acceptButton.click();
    //Mentor SkillBrain20:25 selector smecher o ora si 55 minute
    ////*[contains(text(),'Ai introdus greșit parola sau adresa de email. Te rugăm completează din nou.')
    //test manual model jira
    //Login to demoqa home page
    //Navigate to alerts page
    //Press the first “Click me” button
    //Check the alert pop-up appears
    //Pre-requirements:
    //website is up and running
    //User has full access
    //
    //
    //
    //pagina git hub
    //https://github.com/SkillBrain-com/Helin-QAC-g1-17-11-22
    //script pt pauza scrol buton
    // ((JavascriptExecutor) driver).executeScript("arguments[0].scrollIntoView(true);",page.windowPanel);
// // TODO - Revizuieste codul cand revii la el
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//



























}
